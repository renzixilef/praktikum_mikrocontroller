...
void Sensors::FXAS21002::setStatus(Sensors::FXAS21002Status status) {
    ...
    err = HAL_I2C_Master_Transmit(i2c_dev.get(), FXAS21002_ID, buffer, 1, 10);
    if (err != HAL_OK){
        // Error Handling here
    }
    err = HAL_I2C_Master_Receive(i2c_dev.get(), FXAS21002_ID, &buffer[1], 1, 10);
    if (err != HAL_OK) {
        // Error Handling here
    }
    buffer[1] = (buffer[1] & 0xFC) | (static_cast<uint8_t>(status));
    err = HAL_I2C_Master_Transmit(i2c_dev.get(), FXAS21002_ID, buffer, 2, 10);
    if (err != HAL_OK) {
        // Error Handling here
    }
}
...
void Sensors::FXAS21002::readGyroData() {
    ...
    err = HAL_I2C_Master_Receive(i2c_dev.get(), FXAS21002_ID, buffer, 6, 10);
    if (err != HAL_OK) {
        // Error Handling here
    }
