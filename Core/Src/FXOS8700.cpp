...
void Sensors::FXOS8700::setSensorMode(Sensors::FXOS8700Mode mode) {
    ...
    err = HAL_I2C_Master_Transmit(i2cDev.get(), FXOS8700_ID_TRANSMIT, buffer, 1, HAL_MAX_DELAY);
    if (err != HAL_OK) {
        // Error Handling here
    }
    err = HAL_I2C_Master_Receive(i2cDev.get(), FXOS8700_ID_RECEIVE, &buffer[1], 1, HAL_MAX_DELAY);
    if (err != HAL_OK) {
        // Error Handling here
    }
    buffer[1] = (buffer[1] & 0xFC) | (static_cast<uint8_t>(mode));
    err = HAL_I2C_Master_Transmit(i2cDev.get(), FXOS8700_ID_TRANSMIT, buffer, 2, HAL_MAX_DELAY);
    if (err != HAL_OK) {
        // Error Handling here
    }
}
...
void Sensors::FXOS8700::readMagData() {
    ...
    err = HAL_I2C_Master_Receive(i2cDev.get(), FXOS8700_ID_RECEIVE, buffer, 6, HAL_MAX_DELAY);
    if (err != HAL_OK) {
        // Error Handling here
    }
...
void Sensors::FXOS8700::readAccelData() {
    ...
    HAL_StatusTypeDef err;
    err = HAL_I2C_Master_Transmit(i2cDev.get(), FXOS8700_ID_TRANSMIT, buffer, 1, 10);
    if (err != HAL_OK) {
        // Error Handling here
    }
    err = HAL_I2C_Master_Receive(i2cDev.get(), FXOS8700_ID_RECEIVE, buffer, 6, 10);
    if (err != HAL_OK) {
        // Error Handling here
    }
